...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(
      0,
      `Cannot reach the local API. Is the server running on port 4000? (${
  ...
  if (!response.ok) {
    ...
    const issues = Array.isArray(record['issues'])
      ? (record['issues'] as { path: string; message: string }[])
      : [];
...
export const api = {
  ...
  getAppParties: (params: { tokenId: string; org: string; app: string }) =>
