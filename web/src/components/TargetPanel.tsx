...
export function TargetPanel({
  ...
  const flatParties: AppParty[] = parties.flatMap((party) => [
    party,
    ...(party.childParties ?? []),
  ]);
      ...
      aside={
        metadata ? (
          <span className="badge badge--ok">
            {metadata.metadata.dataTypes?.length ?? 0} data types
          </span>
        ) : undefined
      }
    >
      {savedApps.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          <div className="chips">
            {savedApps.map((saved) => (
              <span
                key={`${saved.org}/${saved.app}`}
                className="chip"
                data-current={saved.org === org && saved.app === app}
              >
      ...
      <div className="row" style={{ marginTop: 12 }}>
        <button
          ...
          className="btn"
          onClick={onProbe}
          disabled={probing || !hasToken || !org || !app}
        >
      ...
      <div className="field" style={{ marginTop: 12 }}>
        ...
        {flatParties.length > 0 && (
          <select
            ...
            value={
              flatParties.some((entry) => String(entry.partyId) === instanceOwnerPartyId)
                ? instanceOwnerPartyId
                : ''
            }
      ...
      {mode === 'existing' && (
        <div className="field" style={{ marginTop: 12 }}>
          ...
          <p className="field__hint">
            Paste the whole <code>510001/99d0632c-…</code> pair and the party id is split out for
            you.
          </p>
        </div>
      )}
