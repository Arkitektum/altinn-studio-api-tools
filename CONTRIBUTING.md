...
- A call to Altinn goes through `altinnFetch`, always. Anything acting on an instance is also recorded by `StepRecorder` so it shows up in the run log, since a write the operator cannot see defeats the point of the tool. The exceptions are `appService` and `localtestClient`, which answer a question rather than change anything and report their findings instead of a step log, and `sweepService`, which writes a great deal and is the one case where recording it would bury the run you were reading under the tool's own traffic.
