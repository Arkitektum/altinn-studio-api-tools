...
export async function postDataToApp(token: string, request: RunRequest): Promise<RunResult> {
    ...
    try {
        const metadata: ApplicationMetadata = await fetchApplicationMetadata(token, request.org, request.app);
    ...
    const unknownDataTypes = request.dataElements.map((element) => element.dataType).filter((id) => dataTypesById.size > 0 && !dataTypesById.has(id));
    ...
    if (mode === "existing") {
        ...
        const response = await recorder.run("Read existing instance", "GET", url, () => altinnFetch({ url, token }));
        if (!response.ok) {
        ...
        const parts: MultipartPart[] = [
            ...
            ...request.dataElements.map((element) => ({
                ...
                contentType: element.contentType ?? resolveContentType(dataTypesById.get(element.dataType), element.content),
        ...
        const hasTemplate = request.instanceTemplate && Object.keys(request.instanceTemplate).length > 0;
        ...
        const body = hasTemplate
            ...
            : undefined;
        const url = hasTemplate ? `${base}/instances` : `${base}/instances?instanceOwnerPartyId=${encodeURIComponent(request.instanceOwnerPartyId)}`;
    ...
    const readBack = await recorder.run("Read back instance", "GET", readBackUrl, () => altinnFetch({ url: readBackUrl, token }));
...
function failed(
    ...
    return {
        ...
        instanceUrl: at?.partyId && at.guid ? instanceUiUrl(at.org, at.app, at.partyId, at.guid) : null,
