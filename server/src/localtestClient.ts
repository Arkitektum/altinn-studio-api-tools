...
export async function createTestUserToken(userId: string, label?: string): Promise<StoredToken> {
  ...
  try {
    ...
    throw new HttpError(
      502,
      `Could not reach LocalTest at ${config.localtestUrl}. Is it running? ` +
  ...
  if (response.status === 404) {
    throw new HttpError(
      404,
      `LocalTest has no test user "${userId}" (404 from ${url}). Check the user id against the ` +
        'LocalTest front page.',
    );
  }
  if (!response.ok) {
