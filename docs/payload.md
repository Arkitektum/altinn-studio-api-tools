...
It is a step rather than a section because it is a thing you do between two other things you do, and the rail says so: `not run` until you press it, then what the service answered, in red where that was errors and amber where it was only warnings. The panel's notice leads with the same counts and takes its colour from the same report, through the same two functions in `chain.ts`, so the two cannot drift. The panel used to colour itself by whether a required document was missing, which is a narrower question than a notice saying what the service made of your submission looks like it is answering, so a report with an error inside the form and four documents it only recommends was green there and red on the rail. What the panel adds is the detail under those counts: the missing documents named and addable, the recommended ones in a line, and what the report says about the form itself rather than about what is attached to it. The errors among those are read out in full whether or not a document is missing, with the rule's own sentence and where in the form it is about. Nothing there can be added from this panel, which is why they used to be counted, but a submission with every document attached and an empty field in the form is still one that would be refused, and "one thing about the form's own content" does not say which thing. The warnings stay a count, since they are the long half of a report the run log has all of. The panel is not rendered at all when no service is configured, and the rail leaves the step out on the same condition, because switched off is not a step you failed to do.
