...
- **A control against the surface it is on**, sunk into it rather than cut out of it. An input, a select, a button and a list row sit on a translucent dark tint, so each one darkens the panel it is on and keeps that panel's hue. They used to be the page colour, opaque, which is right on the page and a hole anywhere else: a near-black slab in the middle of a coloured surface. The tint is cool, like the lift that is its opposite, except under a button wearing a method's colour, where it is that method's: a POST sits on green and a DELETE on red, at a strength that still reads as a surface rather than as a filled button. GET needs nothing of its own, since the cool tint is already its blue.
...
| ----------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| The primary action      | Filled accent                 | The main thing to press here: **Prevalidate**, **Post**, **Get token**, a **Renew** on a token that has expired                                                    |
| Sends a request         | Outlined in its method colour | **Refresh** and **Render pdf** are GET blue, **Sign and submit** is PUT amber, **Start** on a sweep and a **Renew** on a token that has not expired are POST green |
...
So the colour answers "will this talk to Altinn, and how", which is the question worth answering before clicking in a tool whose whole purpose is making requests. Primacy wins where the two disagree: **Post** and **Prevalidate** are filled rather than green, because being the one action a panel is for says more than being a POST.
