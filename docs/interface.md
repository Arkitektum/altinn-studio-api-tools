...
The link whose panel you are looking at is marked as you scroll, so the strip reads as a position as well as a state. That marking is a violet chip with a rule under it. Not the accent, which already means "the next thing to do" and a position is not a thing to do, and not grey, which was tried twice and read as a slightly lighter rectangle both times. Violet means nothing else in the strip, the same argument that lets it mean a subform in one place and an attribute name in another. Application and Party are both marked together, since both are set in Target. Every link is marked this way, the test user included: it scrolls with everything else now, where before it sat in a rail that was always on screen and marking it would have said nothing.
